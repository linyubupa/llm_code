...
from .zigzag_ring_attn.monkey_patch import apply_zigzag_ring_attn_monkey_patch_llama  ,apply_zigzag_ring_attn_monkey_patch_qwen  
...
def apply_seq_parallel_monkey_patch(
    ...
    assert model in ["llama", "qwen"], f"Invalid model: {model}"
    if seq_algo == "data_parallel":
        return
    elif seq_algo == "zigzag_ring_attn" and model == "llama":
        apply_zigzag_ring_attn_monkey_patch_llama()
    elif seq_algo == "zigzag_ring_attn" and model == "qwen":
        apply_zigzag_ring_attn_monkey_patch_qwen()
