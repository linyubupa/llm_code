...
def mask_targets(conversations, targets, tokenizer, conv):
    for conversation, target in zip(conversations, targets):
        ...
        if conv.sep_style == SeparatorStyle.LLAMA3:
            system_turn_seperator = f"<|start_header_id|>system<|end_header_id|>"
            if system_turn_seperator in conversation:
                system_turn = conversation.split("<|eot_id|>")[0]
                system_len = len(
                    tokenizer(system_turn, add_special_tokens=False).input_ids
                )
                cur_len = system_len + 1
                target[:cur_len] = IGNORE_TOKEN_ID
                conversation = "<|eot_id|>".join(conversation.split("<|eot_id|>")[1:])
        turns = conversation.split(user_turn_separator)
        if conv.sep_style == SeparatorStyle.LLAMA3 and turns[0] == "":
            turns = conversation.split(user_turn_separator)[1:]
            turns[0] = user_turn_separator + turns[0]
        for i, turn in enumerate(turns):
            ...
            # print(f"assistant_turn_separator is {assistant_turn_separator}")
            ...
            cur_len += turn_len
        target[cur_len:] = IGNORE_TOKEN_ID
        # print(f"cur_len is {cur_len},total_len is {total_len}")
...
class SupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        self.systems = [example.get("system", "") for example in raw_data]
        self.sources = [example["conversations"] for example in raw_data]
        rank0_print("End load inputs...")
        self.template_id = template_id
        self.tokenizer = tokenizer
        # import time
        # start_time = time.process_time()  # 记录开始CPU时间
        # data_dict = preprocess(sources, tokenizer, template_id, systems=systems)
        # end_time = time.process_time()
        # elapsed_time = end_time - start_time  # 计算CPU经过的时间
        # rank0_print(f"程序CPU运行耗时：{elapsed_time}秒")
        # rank0_print("End preprocess inputs...")
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        data_dict = preprocess(
            ...
            self.tokenizer,
            self.template_id,
            systems=[self.systems[i]],
        )
        # print(f"input_ids is {data_dict['input_ids'][0].tolist()}")
        # print(f"labels is {data_dict['labels'][0].tolist()}")
        # print(f"attention_mask is {data_dict['attention_mask'][0].tolist()}")
        return dict(
